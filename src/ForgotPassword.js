...
export function ForgotPassword() {
  ...
  //condtional rendering useStates
  const [otp, setOtp] = useState(false);
  const [passwordChange, setPasswordChange] = useState(false);
  const [mailAndOtpCheck, setMailAndOtpCheck] = useState(true);

  //  useStates to render..if there is error
  const [mailError, setMailError] = useState(false);
  const [otpError, setOtpError] = useState(false);
  const [newPasswordError, setNewPasswordError] = useState(false);
  const [newPasswordMatchError, setNewPasswordMatchError] = useState(false);
  const [passwordNotUpdateError, setPasswordNotUpdatedError] = useState(false);

  //use states to get and set the data from users
  ...
  const passwordUpdation = () => {
    ...
    const updation = { email, newPassword };
    ...
    fetch("https://crm-nodejs-rv.herokuapp.com/forgotPassword",
      ...
      .then((response) => {
        if (response.status === 400) {
          setPasswordNotUpdatedError(true);
        }
        else {
          history.push("/login")
        }
          ...
          ? (otp)
            ? <Box sx={{ display: "grid" }}>
              ...
              <Box sx={{ display: "flex" }}>
                ...
                <Button sx={{ ...font, marginLeft: "auto" }} color="error" onClick={() => { setOtpError(false);setOtp(false); setPasswordChange(false) }}
                >
                  Cancel
                </Button>
        ...
        {(passwordChange)
          ? <Box sx={{ display: "grid" }}>
            ...
            {(newPasswordMatchError)
              ...
              : ""}
            <Box sx={{ display: "flex" }}>
              <Button sx={font} onClick={() => {passwordUpdation();setPasswordNotUpdatedError(false)}}>Update password</Button>
              <Button sx={{ ...font, marginLeft: "auto" }}
                color="error" 
                onClick={() => { 
                  setOtp(false);
                  setPasswordChange(false);
                  setMailAndOtpCheck(true);

                  setMailError(false);
                  setOtpError(false);
                  setNewPasswordError(false);
                  setNewPasswordMatchError(false);
                  setPasswordNotUpdatedError(false);
               }}>cancel</Button>
            </Box>
            <br />

            {(passwordNotUpdateError)
               ? <Box>
                   <Alert severity='error' sx={font}
                     action={
                       <Button color="error" size="small" onClick={() => setPasswordNotUpdatedError(false)}>
                         ❌
                       </Button>
                     }>
                     password should be in required format with minimum 6 characters
