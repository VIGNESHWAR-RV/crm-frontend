...
export function InputComponent({inputProps={}}){
    ...
    const { label="",type="text",options=[],name="",typing=()=>{},helperText=[],pattern="",required=false,color="primary",sx={},variant="outlined",typingPatternErrorCheck="",blurPatternErrorCheck="",...inputprops} = inputProps;
        ...
        {(type === "select")
         ...
         {(typeof(helperText) !== "string")
             ...
             : <FormHelperText sx={{color:(validate.valid)?"":"",...sx}}>
                {(validate.valid)?helperText :(validate.invalid)?`${(type === "select")?"Select a valid "+name:"Enter a valid "+label}.`:helperText}
               </FormHelperText>
             }
      </FormControl>
    )
